...
} from "@/lib/customers/document-actions";
import type { CustomerDocument } from "@/types/database.types";

export const DOCUMENT_TYPE_LABELS: Record<CustomerDocument["document_type"], string> = {
...
export function CustomerDocumentsPanel({
  ...
  function handleView(doc: CustomerDocument) {
    ...
    startTransition(async () => {
      const result = await getDocumentAccessUrl(customerId, doc.id);
      if (result.url) {
  ...
  function handleDelete(doc: CustomerDocument) {
    setAccessError(null);
    startTransition(async () => {
      const result = await deleteCustomerDocument(customerId, doc.id);
  ...
  return (
    <div className="space-y-4">
      <form
        ref={formRef}
        action={formAction}
        className="grid grid-cols-1 gap-3 rounded-md border p-4 sm:grid-cols-2 md:grid-cols-4"
      >
        ...
        {state.error ? (
          <p role="alert" className="text-sm text-destructive sm:col-span-2 md:col-span-4">
            {state.error}
          </p>
        ) : null}
            ...
            <Card key={doc.id}>
              <CardContent className="flex items-center justify-between gap-3 p-3">
                ...
                <div className="flex shrink-0 gap-1.5">
                  ...
                  {canDelete ? (
                    <Button variant="outline" size="icon-sm" onClick={() => handleDelete(doc)}>
